...
if __name__ == '__main__':
    ...
    for category in df.category.unique():
        ...
        for column in category_df:
            ...
            column_to_category_sums[column][category] = category_df[column].loc[category_df[column] != 0].shape[0]
    ...
    for column in column_to_category_sums:
        ...
        trace = Pie(labels=labels, values=values, textfont=dict(size=30), marker=dict(colors=colors, line=dict(color='#000000', width=0)))
        plotly.offline.plot({
            'data': [trace],
            'layout': Layout(title=column, font=dict(family='Courier New, monospace', size=30, color='#000000'))
        })
